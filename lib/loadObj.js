...
function loadObj(objPath, options) {
    ...
    function isConvex(positions2D) {
        ...
        var currentTurnDirection;
        for (var i=1; i < length-2; ++i) {
            currentTurnDirection = getTurnDirection(positions2D[i], positions2D[i+1], positions2D[i+2]);
            if (turnDirection * currentTurnDirection < 0) {
                return false;
            }
        }
        currentTurnDirection = getTurnDirection(positions2D[length-2], positions2D[length-1], positions2D[0]);
        if (turnDirection * currentTurnDirection < 0) {
            return false;
        }
