 ...
 * @param {String} [options.refine = "ADD"] The refine method of tile.
...
function createSingleTileset(options) {
    ...
    var refine = defaultValue(options.refine, 'ADD');
    var headingPitchRoll = defaultValue(options.headingPitchRoll, [0, 0, 0]);
    var tileTransform = wgs84Transform(longitude, latitude, transHeight, headingPitchRoll);
    ...
    var viewerRequestFactor = defaultValue(options.viewerRequestFactor, undefined);
    ...
    if (viewerRequestFactor) {
        viewerRequestVolume = {
            sphere : [
                offsetX, -offsetY, height / 2 + minHeight,
                Math.sqrt(tileWidth * tileWidth / 4 + tileHeight * tileHeight / 4 + height * height / 4) * viewerRequestFactor
            ]
        };
    }
...
function wgs84Transform(longitude, latitude, height, hpr) {
